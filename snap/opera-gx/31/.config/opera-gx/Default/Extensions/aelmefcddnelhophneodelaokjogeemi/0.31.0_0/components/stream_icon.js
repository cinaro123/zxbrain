...
export class StreamIcon extends HTMLElement {
  ...
  async connectedCallback() {
    ...
    this.shadowRoot.querySelector('.avatarIconDefaultClassic')?.remove();
    this.shadowRoot.querySelector('.gameIconClassic')?.remove();
  ...
  render() {
    ...
    const setStreamIconDetails = (el, text) => {
      if (text) {
        el.textContent = text;
      } else {
    ...
    setStreamIconDetails(nameElement, streamName);
    setStreamIconDetails(titleElement, streamTitle);
    setStreamIconDetails(gameTitleElement, gameitle);
