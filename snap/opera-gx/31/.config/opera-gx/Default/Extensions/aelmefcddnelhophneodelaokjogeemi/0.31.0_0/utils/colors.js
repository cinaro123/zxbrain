...
export class Colors {
  constructor({isBackground = false} = {}) {
    ...
    if (!isBackground && navigator.userAgentData.brands.find(
      b => b.brand === "Opera GX")?.version < 119) {
      document.documentElement.toggleAttribute('data-old', true);
    }
  }
  ...
  registerEvents() {
    ...
    opr.palette.onPaletteChanged.addListener(this.paletteListener);
  }

  cleanup() {
    if (this.paletteListener) {
      opr.palette.onPaletteChanged.removeListener(this.paletteListener);
    }
  }

  async getColor() {
    return new Promise(resolve => {
      opr.palette.getColorHSL('gx_accent_l_100', colorObj => {
  ...
  updatePalette(palette) {
    ...
    const validPalette = palette.filter(color => !invalidColors.has(color));
  ...
  async setBadgeInactive() {
    ...
    // On older opera versions, calling getColor will crash
    if (navigator.userAgentData.brands.find(
        b => b.brand === "Opera GX")?.version < 119) {
      return;
    }
    ...
    opr.sidebarAction.setBadgeTextColor({
      color: this.isLighter(this.getLightness(colorObj)) ? '#000000' : '#FFFFFF',
    });
